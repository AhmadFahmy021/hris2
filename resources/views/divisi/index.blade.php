...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-gray-800">Divisi</h1>
    <a href="/kelola/divisi/create" class="btn btn-primary btn-sm"><i class="fas fa-plus text-white-50"></i> Tambah Divisi</a>
</div>
                        ...
                        <tbody>
                            @foreach ($data as $it)        
                                <tr>
                                    ...
                                    <td>{{$it->divisi}}</td>
                                    <td class="text-center">
                                        <a href="/kelola/divisi/{{Crypt::encrypt($it->id)}}/edit" class="btn btn-primary"><i class="fas fa-pen text-white-50"></i> Edit</a>
                                        <form action="/kelola/divisi/{{Crypt::encrypt($it->id)}}" method="post" class="d-inline">
                                            @csrf
                                            @method('DELETE')
                                                <button type="submit" class="btn btn-danger" onclick="return confirm('Apakah anda yakin ingin menghapus data {{$it->divisi}} \nJika anda yakin ingin menghapus data {{$it->divisi}} silahkan cek apakah data sedang terpakai atau tidak')"><i class="fas fa-trash text-white-50" ></i> Delete</button>
                                        </form>
